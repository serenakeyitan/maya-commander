...
description: >-
  ...
  user TYPES on their phone keypad during a call to Maya, then links this machine.
...
1. **Start it** — run with the Bash tool and show the user the output:
   ...
   It prints a **6-digit code** and the number to call. Give the user the code and tell them:
   *"Call Maya at that number, pass your PIN, then TYPE these six digits on your phone keypad and
   press #."* (Typed on the keypad — not spoken. Typing is exact; it avoids voice mishearing, and
   because only the person holding the phone can type it, it's what binds THIS computer to them.)

2. **Wait for them to call + type the code.** While they do, poll once:
   ...
   computer is now linked. If it prints `PENDING`, they haven't typed the code to Maya yet — that's
...
- The 6-digit code is shown on THIS screen and typed on the phone keypad during the call — it never
  travels by text or voice. It expires in a few minutes; if it lapses, just run `maya connect` again
  for a fresh one.
